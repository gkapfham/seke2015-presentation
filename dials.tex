...
\begin{tikzpicture}
  ...
  \path[->]<7-> node[special,]
    (Requirements) at (0,0)
    {How do these parameters influence efficiency and effectiveness?};
  \path[->]<2-> node[special, below of=Requirements,
    yshift=.5in, xshift=0in]
    (Comment) {More effective test adequacy criteria require addtional runtime};
\end{tikzpicture}%
