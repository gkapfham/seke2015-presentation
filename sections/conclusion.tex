\input{styles/kapfhammer-solarized-tikz-colors.tex}
\begin{frame}[t]{Key Contributions}
  \framesubtitle{\mbox{}}

  \hspace*{-.5in}
  \begin{minipage}{5in}
  \begin{center}

    \begin{minipage}{4.5in}

    \begin{figure}

    \begin{center}

      \begin{tikzpicture}[node distance=1cm, auto,>=stealth, thick]

        \path[use as bounding box] (-2,3.5) rectangle (10,-2);

        \path[->]<1-> node[prochighlight, text width=54ex]
        (Loss) at (4,2.8)
        {\large Search-based test data generation is often highly \\ effective,
        but worst-case time complexity unknown};

        \path[->]<2-> node[proc, below of=Loss, text width=50ex,
          yshift=-.225in, xshift=0in] (Comment) {\large A technique for
          automated doubling experiments};

        \path[->]<3-> node[proc, below of=Comment, text width=54ex,
          yshift=-.1in, xshift=0in] (CommentAgain) {\large Emprical suggestions
          for worst-case time complexity};

        \path[->]<4-> node[proc, below of=CommentAgain, text
          width=50ex, yshift=-.1in, xshift=0in] (CommentOneMore)
          {\large Tradeoffs in search-based test data generation};

        \path[->]<5-> node[prochighlight, below of=CommentOneMore, text
          width=36ex, yshift=-.025in, xshift=0in] (CommentFinal)
        {\large https://github.com/kinneerc/ExpOse};

        %% \path[->]<3-> node[prochighlight, below of=Ambler, text width=48ex,
        %%   yshift=-.5in, xshift=0in] (SchemaAnalyst)
        %% {This paper presents {\em SchemaAnalyst}, a search-based
        %%   system for testing the complex \\ integrity constraints in
        %%   relational schemas};

    \end{tikzpicture}

        \end{center}

        \end{figure}

      \end{minipage}

  \end{center}
  \end{minipage}

\end{frame}

