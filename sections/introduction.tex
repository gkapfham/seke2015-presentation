...
\begin{frame}[label=current]
  ...
  \begin{tikzpicture}
    \path[use as bounding box] (-4.5,1) rectangle (10,-2);
          ...
          (Theoretical) (1.5,.75) {\large \xmarkhide {Theoretical}};

          \path[->]<4-> node[plain,below of=Requirements,
          yshift=-0.0in, xshift=-.75in, minimum width=10ex]
          (Theoretical) {\xmark {Theoretical}};

          \path[->]<5> node[plain,below of=Requirements,
          yshift=-0.0in, xshift=.75in, minimum width=10ex]
                      (Empirical) {\cmarkhide {Empirical}};

          \path[->]<6-> node[plain,below of=Requirements,
          yshift=-0.0in, xshift=.75in, minimum width=10ex]
